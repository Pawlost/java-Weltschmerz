...
public class Calculations {
    public static float[] sqrdmd(float[] map, int size, float rgh)
    {
        ...
        sum = ((map[0]) + ( map[step]) + (map[dy]) + (map[dy + step])) * 0.25f;
        sum = sum + slope * ((random.nextInt(100)  << 1) - 100);
        ...
        sum = ((map[0]) + ( map[step]) + center_sum + center_sum) * 0.25f;
        sum = sum + slope * ((random.nextInt(100)  << 1) - 100);
        ...
        /* Left column. */
        p1 = p0 * size;
        sum = ((map[0]) + ( map[dy]) + center_sum + center_sum) * 0.25f;
        sum = sum + slope * ((random.nextInt(100)  << 1) - 100);
        ...
        while (step > 1)  /* Enter the main loop. */
        {
            ...
            while (p0 < size)
            {
                ...
                sum = sum + slope * ((random.nextInt() << 1) - 100);
